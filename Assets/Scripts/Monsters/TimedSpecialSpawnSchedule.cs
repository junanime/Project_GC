...
namespace Vampire
{
    ...
    /// </summary>
    [System.Serializable]
    public class TimedSpecialSpawnSchedule : RuntimeModule
    {
        ...
        protected override void OnTick()
        {
            ...
            if (entityManager == null || levelBlueprint == null || playerCharacter == null)
            {
                ResolveReferences();
            ...
            if (IsMiniStageSpawnBlocked())
            {
                ...
                {
                    Debug.Log("[TimedSpecialSpawnSchedule] 미니 스테이지 진행 중이라 특수 몬스터 스폰 타이머를 멈춥니다.", Host);
                }
        ...
        public void ResetSpawnerRuntime()
        {
            ...
            if (debugLog)
            {
                Debug.Log("[TimedSpecialSpawnSchedule] 런타임 스폰 테이블을 초기화했습니다.", Host);
            }
        }
        ...
        private void BuildRuntimeSchedule()
        {
            ...
            {
                Debug.Log(
                    $"[TimedSpecialSpawnSchedule] 런타임 스폰 테이블 생성 완료 | " +
                    $"Timed Events: {runtimeTimedSpawnEvents.Count} | " +
                    $"Trap Fields: {runtimeTrapFieldStates.Count}", Host
                );
                ...
                for (int i = 0; i < runtimeTimedSpawnEvents.Count; i++)
                {
                    ...
                    Debug.Log(
                        $"[TimedSpecialSpawnSchedule] Timed 예약 #{i + 1} | " +
                        $"시간: {FormatTime(runtimeEvent.scheduledTime)} | " +
                        $"마릿수: {runtimeEvent.spawnCount} | " +
                        $"Blueprint: {blueprintName} | " +
                        $"Memo: {runtimeEvent.runtimeMemo}", Host
                    );
                }
        ...
        private void ResolveReferences()
        {
            if (levelManager == null)
            {
                ...
                {
                    Debug.Log("[TimedSpecialSpawnSchedule] LevelManager를 자동으로 찾았습니다.", Host);
                }
            }
            ...
            if (debugLog)
            {
                ...
                {
                    Debug.LogWarning(
                        "[TimedSpecialSpawnSchedule] EntityManager를 아직 찾지 못했습니다.\nLevelManager 연결을 확인하세요.", Host
                    );
                }
                ...
                {
                    Debug.LogWarning(
                        "[TimedSpecialSpawnSchedule] LevelBlueprint를 아직 찾지 못했습니다. LevelManager 초기화 순서를 확인하세요.", Host
                    );
                }
                ...
                {
                    Debug.LogWarning(
                        "[TimedSpecialSpawnSchedule] Player Character를 아직 찾지 못했습니다. Player 태그 또는 Character 컴포넌트를 확인하세요.", Host
                    );
                }
        ...
        private IEnumerator SpawnTimedEventRoutine(RuntimeTimedSpawnEvent runtimeEvent)
        {
            ...
            if (debugLog)
            {
                ...
                Debug.Log(
                    $"[TimedSpecialSpawnSchedule] Timed Spawn 시작 | " +
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (debugLog)
                {
                    ...
                    Debug.Log(
                        $"[TimedSpecialSpawnSchedule] Timed Spawn 완료 " +
                        $"({i + 1}/{count}) | Monster: {monsterName} | Memo: {runtimeEvent.runtimeMemo}", Host
                    );
                }
        ...
        private void ActivateTrapField(RuntimeTrapFieldState state)
        {
            if (IsMiniStageSpawnBlocked())
            {
                if (logMiniStageBlock)
                {
                    Debug.Log("[TimedSpecialSpawnSchedule] 미니 스테이지 진행 중이라 Trap Field 활성화를 막았습니다.", Host);
                }
            ...
            {
                Debug.Log(
                    $"[TimedSpecialSpawnSchedule] Trap Field 활성화 | " +
                    $"현재 시간: {FormatTime(elapsedTime)} | " +
                    $"Memo: {entry.memo} | " +
                    $"MaxActiveTraps: {entry.maxActiveTraps} | " +
                    $"RespawnDelay: {entry.respawnDelay}", Host
                );
            }
            ...
            {
                Debug.LogError(
                    $"[TimedSpecialSpawnSchedule] Trap Field 활성화 실패 | Memo: {entry.memo}", Host
                );
            ...
            if (!entry.spawnInitialTrapsOnActivate)
            {
                ...
                {
                    Debug.Log(
                        $"[TimedSpecialSpawnSchedule] Spawn Initial Traps On Activate가 꺼져 있어서 초기 함정을 스폰하지 않습니다.\nMemo: {entry.memo}", Host
                    );
                }
            ...
            {
                Debug.LogWarning(
                    $"[TimedSpecialSpawnSchedule] Max Active Traps가 0 이하라서 함정을 스폰하지 않습니다.\nMemo: {entry.memo}", Host
                );
        ...
        private bool CanSpawnTrap(RuntimeTrapFieldState state)
        {
            ...
            if (entityManager == null)
            {
                ...
                {
                    Debug.LogError(
                        "[TimedSpecialSpawnSchedule] EntityManager의 SpawnMonster 메서드를 찾지 못했습니다.\n함정 몬스터를 지정 위치에 스폰할 수 없습니다.", Host
                    );
            ...
            if (playerCharacter == null)
            {
                ...
                {
                    Debug.LogError(
                        "[TimedSpecialSpawnSchedule] Player Character를 찾지 못했습니다. 함정 몬스터 위치 계산을 할 수 없습니다.", Host
                    );
            ...
            {
                Debug.LogWarning(
                    $"[TimedSpecialSpawnSchedule] Trap Field Maintain 모드인데 Monster Blueprint가 TrapMonsterBlueprint 타입이 아닙니다. " +
                    $"그래도 스폰은 시도합니다.\nBlueprint: {entry.monsterBlueprint.name} | Memo: {entry.memo}", Host
                );
            }
        ...
        private void SpawnOneTrap(RuntimeTrapFieldState state)
        {
            if (IsMiniStageSpawnBlocked())
            {
                if (logMiniStageBlock)
                {
                    Debug.Log("[TimedSpecialSpawnSchedule] 미니 스테이지 진행 중이라 함정 스폰을 막았습니다.", Host);
                }
            ...
            if (state.activeTraps.Count >= maxActiveTraps)
            {
                ...
                {
                    Debug.Log(
                        $"[TimedSpecialSpawnSchedule] 최대 함정 수에 도달해서 함정을 추가 스폰하지 않습니다.\n" +
                        $"현재 {state.activeTraps.Count}/{maxActiveTraps} | Memo: {entry.memo}", Host
                    );
                }
            ...
            {
                Debug.Log(
                    $"[TimedSpecialSpawnSchedule] 함정 스폰 시도 | " +
            ...
            {
                Debug.LogError(
                    $"[TimedSpecialSpawnSchedule] 함정 스폰 실패: SpawnMonster 결과가 NULL입니다.\n" +
            ...
            {
                Debug.Log(
                    $"[TimedSpecialSpawnSchedule] 함정 스폰 완료 | " +
        ...
        private Monster SpawnMonsterAtPosition(int poolIndex, Vector2 position, MonsterBlueprint blueprint, float hpBuff)
        {
            return entityManager != null ? entityManager.SpawnMonster(poolIndex, position, blueprint, hpBuff) : null;
        }
        ...
        private void OnAnyTrapKilled(Monster killedTrap)
        {
            ...
            for (int i = 0; i < runtimeTrapFieldStates.Count; i++)
            {
                ...
                {
                    Debug.Log(
                        $"[TimedSpecialSpawnSchedule] 함정 사망 감지 | " +
        ...
        private bool CanSpawnCommon(TimedSpawnEntry entry, out int resolvedPoolIndex)
        {
            ...
            if (entityManager == null || levelBlueprint == null)
            {
                ...
                {
                    Debug.LogError(
                        "[TimedSpecialSpawnSchedule] EntityManager 또는 LevelBlueprint가 없어 몬스터를 스폰할 수 없습니다.", Host
                    );
            ...
            if (entry == null)
            {
                Debug.LogError("[TimedSpecialSpawnSchedule] Spawn Entry가 NULL입니다.", Host);
                return false;
            }
            ...
            {
                Debug.LogError(
                    $"[TimedSpecialSpawnSchedule] Monster Blueprint가 비어 있습니다.\nMemo: {entry.memo}", Host
                );
        ...
        private bool TryResolvePoolIndex(TimedSpawnEntry entry, out int resolvedPoolIndex)
        {
            ...
            if (entry.autoResolvePoolIndex)
            {
                ...
                {
                    Debug.LogError(
                        $"[TimedSpecialSpawnSchedule] 자동 poolIndex 탐색 실패 | " +
                        $"Blueprint={entry.monsterBlueprint.name} | Memo={entry.memo}\n" +
                        "Level Blueprint의 Monster Settings에 해당 Blueprint가 등록되어 있는지 확인하세요.", Host
                    );
            ...
            {
                Debug.LogError(
                    $"[TimedSpecialSpawnSchedule] 잘못된 poolIndex입니다.\n" +
                    $"PoolIndex={resolvedPoolIndex} | " +
                    $"Blueprint={entry.monsterBlueprint.name} | " +
                    $"Memo={entry.memo}", Host
                );
