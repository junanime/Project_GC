...
namespace Vampire
{
    ...
    public class EliteMonsterSpawner : RuntimeModule
    {
        ...
        protected override void OnAwake()
        {
            ...
            if (debugLog)
            {
                Debug.Log("[EliteMonsterSpawner] Awake 호출됨 - 컴포넌트 활성 상태 확인 완료", Host);
            }
        }
        ...
        protected override void OnTick()
        {
            ...
            if (IsMiniStageSpawnBlocked())
            {
                ...
                {
                    Debug.Log("[EliteMonsterSpawner] 미니 스테이지 진행 중이라 엘리트 스폰 타이머를 멈춥니다.", Host);
                }
        ...
        private void TrySpawnFromPhase(EliteSpawnPhase phase)
        {
            if (IsMiniStageSpawnBlocked())
            {
                if (logMiniStageBlock)
                {
                    Debug.Log("[EliteMonsterSpawner] 미니 스테이지 진행 중이라 엘리트 즉시 스폰을 막았습니다.", Host);
                }
            ...
            if (!ready)
            {
                if (debugLog)
                {
                    Debug.LogWarning("[EliteMonsterSpawner] 아직 준비되지 않아 엘리트를 스폰하지 않습니다.", Host);
                }
            ...
            for (int i = 0; i < spawnCount; i++)
            {
                ...
                if (!TrySelectEliteFlatIndex(phase, selectedEliteIndicesThisWave, out eliteFlatIndex))
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"[EliteMonsterSpawner] 엘리트 후보 선택 실패 | Phase: {phase.phaseName}", Host
                        );
                    }
            ...
            out int eliteFlatIndex)
        {
                ...
                phase.normalToEliteMappings.Length == 0)
            {
                ...
                {
                    Debug.LogWarning(
                        $"[EliteMonsterSpawner] {phase.phaseName}: Normal To Elite Mappings가 비어 있습니다.", Host
                    );
                }
        ...
        private Monster TrySpawnEliteByFlatIndex(int flatIndex, float additionalHpBuff)
        {
            ...
            {
                Debug.LogWarning($"[EliteMonsterSpawner] 잘못된 flat index입니다: {flatIndex}", Host);
                return null;
            }

            if (entry.blueprint == null)
            {
                Debug.LogWarning($"[EliteMonsterSpawner] MonsterBlueprint가 비어 있습니다. flatIndex={flatIndex}", Host);
                return null;
            }
            ...
            if (requireEliteMonsterBlueprint && !(entry.blueprint is EliteMonsterBlueprint))
            {
                Debug.LogWarning(
                    ...
                    $"name={entry.blueprint.name}, type={entry.blueprint.GetType().Name}. 스폰하지 않습니다.", Host
                );
            ...
            {
                Debug.LogWarning($"[EliteMonsterSpawner] 엘리트 스폰 실패. flatIndex={flatIndex}", Host);
                return null;
            }
        ...
        private bool ValidatePrefabAndBlueprintCompatibility(FlatMonsterEntry entry)
        {
            ...
            {
                Debug.LogWarning(
                    $"[EliteMonsterSpawner] flatIndex={entry.flatIndex}의 Monsters Prefab이 비어 있습니다.", Host
                );
            ...
            if (entry.blueprint is MeleeMonsterBlueprint && !(prefabMonster is MeleeMonster))
            {
                Debug.LogWarning(
                    ...
                    $"MeleeMonsterBlueprint 계열은 MeleeMonster 프리팹에 넣어야 합니다.", Host
                );
        ...
        private void OnEliteKilled(Monster killedMonster)
        {
            ...
            {
                Debug.Log(
                    $"[EliteMonsterSpawner] 엘리트 사망 감지 | Active: {activeElites.Count}", Host
                );
            }
        }
        ...
        private void LogMonsterIndexTable()
        {
            ...
            {
                Debug.LogWarning(
                    "[EliteMonsterSpawner] Monster Index Table 출력 실패: LevelManager 또는 LevelBlueprint가 없습니다.", Host
                );
            ...
            if (levelBlueprint.monsters == null)
            {
                Debug.LogWarning("[EliteMonsterSpawner] LevelBlueprint.monsters가 비어 있습니다.", Host);
                return;
            }
