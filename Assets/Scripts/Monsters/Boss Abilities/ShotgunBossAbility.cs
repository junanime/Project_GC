...
namespace Vampire
{
    public class ShotgunBossAbility : BossAbility
    {
        ...
        protected void LaunchBullets()
        {
            ...
            {
                float theta =
                    (spreadAngle * i) / bulletCount -
                    spreadAngle / 2;

                Vector2 direction = new Vector2(
                    initialDir.x * Mathf.Cos(Mathf.Deg2Rad * theta) -
                    initialDir.y * Mathf.Sin(Mathf.Deg2Rad * theta),

                    initialDir.x * Mathf.Sin(Mathf.Deg2Rad * theta) +
                    initialDir.y * Mathf.Cos(Mathf.Deg2Rad * theta)
                );
