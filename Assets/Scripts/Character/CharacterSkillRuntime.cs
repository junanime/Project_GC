...
namespace Vampire
{
    ...
    public sealed class CharacterSkillRuntime : MonoBehaviour
    {
        ...
        public void Initialize(Character character) { owner=character; aura=gameObject.AddComponent<SkillWindVisual>(); aura.Bind(this); }
        public void DashFinished() { if (Definition != null && owner.IsAlive) PassiveRemaining=Definition.passiveDuration; }
        public int ProjectileCount(int count) => Mathf.Max(1,count) * (Active ? 2 : 1);
