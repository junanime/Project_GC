...
{
    [System.Serializable]
    public class StageEventSchedule : RuntimeModule
    {
        protected override void OnSuspended()
        {
            StageEventRuntimeModifiers.ResetCoinModifiers();
            if (acidRainScreenVFX != null) acidRainScreenVFX.StopImmediately();
        }
        protected override void OnResumed()
        {
            if (activeAcidRainEventCount > 0 && acidRainScreenVFX != null) acidRainScreenVFX.PlayRain();
        }
        protected override void OnModuleDestroy()
        {
            foreach (var entry in acidSecretionEvents)
                if (entry?.activePuddles != null)
                    foreach (var puddle in entry.activePuddles) if (puddle != null) Destroy(puddle);
        }
        [System.Serializable]
